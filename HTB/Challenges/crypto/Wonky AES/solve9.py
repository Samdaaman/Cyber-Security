...
counter = 0
r11_options: list[list[tuple[int,int,int,int]]] = [list() for _ in range(4)]

def do_work(args: tuple[int, list[int]]):
    c0, col_xor_result = args
    r11_options: list[tuple[int,int,int,int]] = []
        ...
        for c2 in range(256):
            ...
            for c3 in range(256):
                ...
                if test_val.count(0) == 3:
                    r11_options.append((c0,c1,c2,c3))
    return r11_options

while any(len(x) == 0 for x in r11_options):
    ...
    with multiprocessing.Pool() as pool:
        for result in pool.imap_unordered(do_work, [(i, col_xor_result) for i in range(256)]):

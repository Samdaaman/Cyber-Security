import math
from pwn import remote, process
from Crypto.Util.strxor import strxor
from icecream import ic
import lib

sbox = [
    ,
]
inv_sbox = [sbox.index(i) for i in range(256)]

mix_col_lookup: dict[int, tuple[int, int, list[int]]] = {}
mix_col_xor_options: list[set[int]] = [set() for _ in range(4)]
for i in range(4):
    col = [0 for _ in range(4)]
    for c in range(256):
        col[i] = c
        lib.mix_single_column(col)
        idx = sum(col[j] << (24 - 8*j) for j in range(4))
        mix_col_lookup[idx] = (c, i, [])
        for j in range(4):
            mix_col_xor_options[j].add(col[j])

io = process('enc_fault2', stderr=None) # type: ignore

possible_lrnd_keys_all = [set(i for i in range(256)) for _ in range(16)]

while any(len(x) > 1 for x in possible_lrnd_keys_all):
    io.sendlineafter(b': ', b'y')
    correct = bytes.fromhex(io.recvline_contains(b': ').decode().split(':')[1])
    faulted = bytes.fromhex(io.recvline_contains(b': ').decode().split(':')[1])
    xor_result = strxor(correct, faulted)

    for buf in [correct, faulted, xor_result]:
        for i in range(4):
            print(buf[i*4:(i+1)*4].hex())
        print()

    col_num = 0
    for col_num in range(4):
        if xor_result[col_num] != 0:
            break

    col_correct = [correct[i*4 + (col_num-i)%4] for i in range(4)]
    col_fault = [faulted[i*4 + (col_num-i)%4] for i in range(4)]
    col_xor_result = [xor_result[i*4 + (col_num-i)%4] for i in range(4)]
    possible_lrnd_keys_col = [possible_lrnd_keys_all[i*4 + col_num] for i in range(4)]
    ic([hex(c) for c in col_xor_result])

    if math.prod(len(x) for x in possible_lrnd_keys_col) > 2*16:
        for i in range(4):
            possible_lrnd_keys: set[int] = set()
            for pre_sbox_char in range(256):
                for xor in mix_col_xor_options[i]:
                    pre_sbox_char_xord = pre_sbox_char ^ xor
                    post_sbox_char = sbox[pre_sbox_char]
                    post_sbox_char_xord = sbox[pre_sbox_char_xord]
                    if post_sbox_char ^ post_sbox_char_xord == col_xor_result[i]:
                        possible_lrnd_keys.add(col_correct[i] ^ post_sbox_char)
            possible_lrnd_keys_all[i*4 + col_num].intersection_update(possible_lrnd_keys)

    else:
        for r0 in possible_lrnd_keys_col[0]:
            for r1 in possible_lrnd_keys_col[1]:
                for r2 in possible_lrnd_keys_col[2]:
                    for r3 in possible_lrnd_keys_col[3]:
                        post_sbox = [
                            col_correct[0] ^ r0,
                            col_correct[1] ^ r1,
                            col_correct[2] ^ r2,
                            col_correct[3] ^ r3,
                        ]
                        post_sbox_x = [
                            col_fault[0] ^ r0,
                            col_fault[1] ^ r1,
                            col_fault[2] ^ r2,
                            col_fault[3] ^ r3,
                        ]
                        pre_sbox = [inv_sbox[x] for x in post_sbox]
                        pre_sbox_x = [inv_sbox[x] for x in post_sbox_x]
                        pre_sbox_xor_result = [i^j for i,j in zip(pre_sbox, pre_sbox_x)]
                        idx = sum(pre_sbox_xor_result[j] << (24 - 8*j) for j in range(4))


    ic([len(x) for x in possible_lrnd_keys_all])

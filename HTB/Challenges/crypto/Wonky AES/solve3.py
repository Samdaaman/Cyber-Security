from pwn import remote, process
from Crypto.Util.strxor import strxor
from icecream import ic
import lib

sbox = [
    ,
]
inv_sbox = [sbox.index(i) for i in range(256)]

mix_col_lookup: dict[int, dict[int, dict[int, dict[int, tuple[int, int]]]]] = {}
for i in range(4):
    col = [0 for _ in range(4)]
    for c in range(256):
        col[i] = c
        lib.mix_single_column(col)
        if col[0] not in mix_col_lookup:
            mix_col_lookup[col[0]] = {}
        if col[1] not in mix_col_lookup[col[0]]:
            mix_col_lookup[col[0]][col[1]] = {}
        if col[2] not in mix_col_lookup[col[0]][col[1]]:
            mix_col_lookup[col[0]][col[1]][col[2]] = {}
        mix_col_lookup[col[0]][col[1]][col[2]][col[3]] = (c, i)

io = process('enc_fault2', stderr=None) # type: ignore

io.sendlineafter(b': ', b'y')
correct = bytes.fromhex(io.recvline_contains(b': ').decode().split(':')[1])
faulted = bytes.fromhex(io.recvline_contains(b': ').decode().split(':')[1])
xord = strxor(correct, faulted)

for buf in [correct, faulted, xord]:
    for i in range(4):
        print(buf[i*4:(i+1)*4].hex())
    print()

col_num = 0
for col_num in range(4):
    if xord[col_num] != 0:
        break

col_xord = [xord[i*4 + (col_num-i)%4] for i in range(4)]
ic([hex(c) for c in col_xord])

possible_post_sbox_chars: list[list[int]] = [[] for _ in range(4)]

# for i in range(4):
#     for pre_sbox_char in range(256):
#         for xor in mix_col_xor_options[i]:
#             pre_sbox_char_xord = pre_sbox_char ^ xor
#             post_sbox_char = sbox[pre_sbox_char]
#             post_sbox_char_xord = sbox[pre_sbox_char_xord]
#             if post_sbox_char ^ post_sbox_char_xord == col_xord[i]:
#                 possible_post_sbox_chars[i].append(post_sbox_char)

possible_lrnd_keys = [set(i for i in range(256)) for _ in range(4)]

for c0, d0 in mix_col_lookup.items():
    for c1, d1 in d0.items():
        for c2, d2 in d1.items():
            for c3, d3 in d2.items():
                for pre_sbox_char in range(256):
                    for i in range(4):
                        pre_sbox_char_xord = pre_sbox_char ^ 
                        post_sbox_char = sbox[pre_sbox_char]
                        post_sbox_char_xord = sbox[pre_sbox_char_xord]
                        if post_sbox_char ^ post_sbox_char_xord == col_xord[i]:
                            possible_post_sbox_chars[i].append(post_sbox_char)

ic([len(x) for x in possible_post_sbox_chars])



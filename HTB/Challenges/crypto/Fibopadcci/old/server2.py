...
class SuperSecureEncryption: # This should be unbreakable!
    ...
    def encrypt(self, data):
        print(f'data unpadded {[c for c in data]}')
        data = pad(data)
        print(f'data padded {[c for c in data]}')
        ...
        for block in blocks:
            print(f's0 is {[c for c in block]}')

            s1 = xor(lb_cipher, block)
            print(f's1 is {[c for c in s1]}')
            
            enc = self.cipher.encrypt(s1)
            print(f's2 is {[c for c in enc]}')
            
            enc = xor(enc, lb_plain)
            print(f's3 is {[c for c in enc]}')
    ...
    def decrypt(self, data, a, b):
        ...
        for block in data:
            print(f's3 is {[c for c in block]}')

            s2 = xor(block, lb_plain)
            print(f's2 is {[c for c in s2]}')

            dec = self.cipher.decrypt(s2)
            print(f's1 is {[c for c in dec]}')

            dec = xor(dec, lb_cipher)
            print(f's0 is {[c for c in dec]}')

            output += dec
            lb_plain = dec
            lb_cipher = block
        print([c for c in output])
        if checkpad(output):
            return output
        else:
            return None

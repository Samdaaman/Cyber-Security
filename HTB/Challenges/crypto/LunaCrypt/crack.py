...
def ESwapChar(char):
    ...
    # return strchr(bitbor(bitxor(THIS_MSB, 0x0), bitxor(bitlst(THIS_LSB, 4), 0x0)))
    return strchr(bitbor(bitxor(THIS_MSB, 0x0D), bitxor(bitlst(THIS_LSB, 4), 0xB0)))

def rev_ESwapChar(char):
    char = ValidateChar(char)
    THIS_MSB = bitext(char, 4, 4)
    THIS_LSB = bitext(char, 0, 4)
    # return strchr(bitbor(bitxor(THIS_MSB, 0x0), bitxor(bitlst(THIS_LSB, 4), 0x0)))
...
def decrypt_char(char, flag):
    ...
    if CheckFlag(flag, FL_NEGATE):
        char = NegateChar(char)
    if CheckFlag(flag, FL_SWAPBYTES):
        char = rev_ESwapChar(char)
    return char if isinstance(char, str) else chr(char)
...
with open('output.txt') as fh:
    ...
    for i in range(len(ints) // 2):
        chars.append(ints[i*2])
        flags.append(ints[i*2+1] ^ 0x4A)

    pt = []
    for i in range(len(chars)):
        pt.append(decrypt_char(chars[i], flags[i]))
    
    print(''.join(pt))

t = 0x3c
print(hex(t))
print(hex(ord(rev_ESwapChar(ESwapChar(t)))))
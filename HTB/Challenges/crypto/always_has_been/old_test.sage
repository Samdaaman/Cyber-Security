...
def main():
    ...
    Fpow.<w> = GF(2^8)
    poly_byte = sbox.interpolation_polynomial(Fpow)
    ...
    pt_0 = os.urandom(32)
    pt_0_sub = substitute(sbox, pt_0)
    pt_0_per = permute(pt_0_sub)
    pt_1 = xor_arr(pt_0_per, pt_0)

    pt_0_fsmall = [Fpow.fetch_int(i) for i in pt_0]
    pt_0_sub_fsmall = [poly_byte(i) for i in pt_0_fsmall]
    pt_0_sub_fsmall_ints = [i.integer_representation() for i in pt_0_sub_fsmall]
    assert pt_0_sub == bytes(pt_0_sub_fsmall_ints)

    pt_0_suba_fsmall = [a * i for i in pt_0_fsmall]
    pt_0_subb_fsmall = [b] * 32
    pt_0_suba_fsmall_ints = [i.integer_representation() for i in pt_0_suba_fsmall]
    pt_0_subb_fsmall_ints = [i.integer_representation() for i in pt_0_subb_fsmall]
    pt_0_sub_axorb_fsmall = xor_arr(pt_0_suba_fsmall_ints, pt_0_subb_fsmall_ints)
    assert pt_0_sub == pt_0_sub_axorb_fsmall
    ...
    X_test = vector(F, block_to_bits(pt_0))
    Y_test = A * X_test + B
    ...
    for i in range(256):
        ...
        P[inbyte*8 + inbit, outbyte*8 + outbit] = 1

    X_test = vector(F, block_to_bits(pt_0_sub))
    Y_test = X_test*P
    Y_test_bytes = bits_to_block(Y_test)
    ic(Y_test_bytes.hex())
    ic(pt_0_per.hex())

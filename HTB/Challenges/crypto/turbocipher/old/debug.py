...
def main(s):
    while True:
        ...
        try:
            for i in range(10):
                assert turbonacci(i, p, b, c) == lib.fast_turbonacci(i, p, b, c)
                assert turbocrypt(i, -1, int) == lib.fast_turbocrypt(i, -1, int)
    ...
    if otp != lib.fast_turbonacci(nonce, p, b, c):
    ...
    while (option := menu(s)) != 3:
        if option == 1:
            ...
            sendMessage(
                s, f"ct = {lib.fast_turbocrypt(bytes_to_long(FLAG), k, f) % p}\n")

        if option == 2:
            pt = receiveMessage(s, "pt = ").strip().encode()
            assert len(pt) * 8 <= p.bit_length()
            sendMessage(
                s, f"ct = {lib.fast_turbocrypt(bytes_to_long(pt), k, f) % p}\n")
